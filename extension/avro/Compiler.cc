...
static NodePtr makeNode(const string &t, SymbolTable &st, const string &ns) {
    ...
    throw duckdb::InvalidInputException("Unknown type: %s", n.fullname());
}
...
void ensureType(const Entity &e, const string &name) {
    if (e.type() != json::type_traits<T>::type()) {
        throw duckdb::InvalidInputException("Json field \"%s\" is not a %s: %s", name, json::type_traits<T>::name(), e.toString());
    }
}
...
static void unescape(string &s) {
	size_t start_pos = 0;
	while ((start_pos = s.find("\\\"", start_pos, 2)) != string::npos) {
		s.replace(start_pos, 2, 1, '"');
		++start_pos;
	}
}
...
static void assertType(const Entity &e, EntityType et) {
    if (e.type() != et) {
        throw duckdb::InvalidInputException("Unexpected type for default value: "
                                      "Expected %s, but found %s in line %d",
...
static GenericDatum makeGenericDatum(NodePtr n,
    ...
    switch (t) {
            ...
            for (size_t i = 0; i < n->leaves(); ++i) {
                ...
                if (it == v.end()) {
                    throw duckdb::InvalidInputException(
                                        "No value found in default for %s",
                                    n->nameAt(i));
                }
                result.setFieldAt(i,
        ...
        default: throw duckdb::InvalidInputException("Unknown type: %d", t);
    }
}
...
static LogicalType makeLogicalType(const Entity &e, const Object &m) {
    ...
    if (typeField == "decimal") {
        ...
        } catch (duckdb::InvalidInputException &ex) {
...
static NodePtr makeEnumNode(const Entity &e,
    ...
    for (const auto &it : v) {
        if (it.type() != json::EntityType::String) {
            throw duckdb::InvalidInputException("Enum symbol not a string: %s", it.toString());
        }
...
static NodePtr makeFixedNode(const Entity &e,
    ...
    if (v <= 0) {
        throw duckdb::InvalidInputException("Size for fixed is not positive: %s", e.toString());
    }
    NodePtr node =
...
static NodePtr makeNode(const Entity &e, const Object &m,
    ...
    throw duckdb::InvalidInputException("Unknown type definition: %s",
                    e.toString());
}
...
static NodePtr makeNode(const json::Entity &e, SymbolTable &st, const string &ns) {
    switch (e.type()) {
        ...
        default: throw duckdb::InvalidInputException("Invalid Avro type: %s", e.toString());
    }
}
json::Object::const_iterator findField(const Entity &e, const Object &m, const string &fieldName) {
    auto it = m.find(fieldName);
    if (it == m.end()) {
        throw duckdb::InvalidInputException("Missing Json field \"%s\": %s", fieldName, e.toString());
    } else {
        return it;
    }
}
...
void compileJsonSchema(std::istream &is, ValidSchema &schema) {
    if (!is.good()) {
        throw duckdb::InvalidInputException("Input stream is not good");
    }
