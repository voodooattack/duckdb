...
protected:
    ...
    void doAddName(const std::string &name) override {
        if (!nameIndex_.add(name, leafNameAttributes_.size())) {
            throw duckdb::InvalidInputException("Cannot add duplicate name: %s", name);
        }
...
public:
    ...
    NodePtr getNode() const {
        ...
        if (!node) {
            throw duckdb::InvalidInputException("Could not follow symbol %s", std::string(name()));
        }
        return node;
    }
        ...
        for (size_t i = 0; i < leafNameAttributes_.size(); ++i) {
            if (!nameIndex_.add(leafNameAttributes_.get(i), i)) {
                throw duckdb::InvalidInputException(
                                    "Cannot add duplicate field: %s", leafNameAttributes_.get(i));
            }
        }
    }
...
public:
    ...
    NodeEnum(const HasName &name, const LeafNames &symbols) : NodeImplEnum(AVRO_ENUM, name, NoLeaves(), symbols, NoSize()) {
        for (size_t i = 0; i < leafNameAttributes_.size(); ++i) {
            if (!nameIndex_.add(leafNameAttributes_.get(i), i)) {
                throw duckdb::InvalidInputException("Cannot add duplicate enum: %s", leafNameAttributes_.get(i));
            }
        }
    }
...
NodeImpl<A, B, C, D>::setLeafToSymbolic(size_t index, const NodePtr &node) {
    if (!B::hasAttribute) {
        throw duckdb::InvalidInputException("Cannot change leaf node for nonexistent leaf");
    }

    auto &replaceNode = const_cast<NodePtr &>(leafAttributes_.get(index));
    if (replaceNode->name() != node->name()) {
        throw duckdb::InvalidInputException("Symbolic name does not match the name of the schema it references");
    }
...
inline NodePtr resolveSymbol(const NodePtr &node) {
    if (node->type() != AVRO_SYMBOLIC) {
        throw duckdb::InvalidInputException("Only symbolic nodes may be resolved");
    }
