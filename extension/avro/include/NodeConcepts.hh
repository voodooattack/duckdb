...
struct NoAttribute {
    ...
    void add(const Attribute & /* attr */) {
        ...
        // just in case
        throw duckdb::InvalidInputException("This type does not have attribute");
    }
...
struct SingleAttribute {
    ...
    const Attribute &get(size_t index = 0) const {
        if (index != 0) {
            throw duckdb::InvalidInputException("SingleAttribute has only 1 value");
        }
        return attr_;
    }
...
struct NameIndexConcept {
    ...
    bool lookup(const std::string &name, size_t &index) const {
        throw duckdb::InvalidInputException("Name index does not exist");
    }
