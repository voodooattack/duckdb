...
static bool validate(const NodePtr &node, SymbolMap &symbolMap) {
    if (!node->isValid()) {
        throw duckdb::InvalidInputException("Schema is invalid, due to bad node of type %d", node->type());
    }
    ...
    if (node->hasName()) {
        ...
        if (node->type() == AVRO_SYMBOLIC) {
            if (!found) {
                throw duckdb::InvalidInputException("Symbolic name \"%s\" is unknown", std::string(node->name()));
            }
...
string ValidSchema::compactSchema(const string &schema) {
    ...
    if (insideQuote) {
        throw duckdb::InvalidInputException("Schema is not well formed with mismatched quotes");
    }
