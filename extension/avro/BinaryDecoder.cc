...
bool BinaryDecoder::decodeBool() {
    ...
    throw duckdb::InvalidInputException("Invalid value for bool: %d", v);
}
...
int32_t BinaryDecoder::decodeInt() {
    ...
    if (val < INT32_MIN || val > INT32_MAX) {
        throw duckdb::InvalidInputException(
            "Value out of range for Avro int: %d", val);
    }
...
size_t BinaryDecoder::doDecodeLength() {
    ...
    if (len < 0) {
        throw duckdb::InvalidInputException(
            "Cannot have negative length: %d", len);
    }
    return len;
}
...
int64_t BinaryDecoder::doDecodeLong() {
    ...
    do {
        if (shift >= 64) {
            throw duckdb::InvalidInputException("Invalid Avro varint");
        }
        u = in_.read();
