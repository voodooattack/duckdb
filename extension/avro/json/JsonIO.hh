...
private:
    ...
    Token tryString();
    static duckdb::InvalidInputException unexpected(unsigned char ch);
    char next();
...
public:
    ...
    void drain() {
        if (!stateStack.empty() || peeked) {
            throw duckdb::InvalidInputException("Invalid state for draining");
        }
        in_.drain(hasNext);
        hasNext = false;
    }
        ...
        for (const char *p = b; p != e; p++) {
            if ((*p & 0x80) != 0) {
                ...
                if (binary) {
                    ...
                    throw duckdb::InvalidInputException("Invalid UTF-8 sequence");
                } else {
                    int more = 1;
                    uint32_t value;
                    if ((*p & 0x20) != 0) {
