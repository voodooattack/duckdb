...
void JsonParser::expectToken(Token tk) {
    if (advance() != tk) {
        ...
        throw duckdb::InvalidInputException(oss.str());
    }
}
    ...
    for (;;) {
        switch (state) {
            ...
            case 7:
                ...
                break;
            default:
                throw duckdb::InvalidInputException("Unexpected JSON parse state");
        }
        if (state == 1 || state == 2 || state == 4 || state == 7) {
            ...
            if (hasNext) {
                ...
                throw duckdb::InvalidInputException("Unexpected EOF");
            }
        }
    }
}
...
duckdb::InvalidInputException JsonParser::unexpected(unsigned char c) {
    std::ostringstream oss;
    oss << "Unexpected character in json " << toHex(c / 16) << toHex(c % 16);
    return duckdb::InvalidInputException(oss.str());
}
